...
- Стиль лендинга v2 («полевой журнал»): центрированная белая карточка `bg-bg-elevated` + hairline `border-edge` + скругления, лого `ibo●` над карточкой (ссылка на `/`), заголовок `font-display` (Playfair Display)
- Лейблы полей — `font-mono` uppercase с letterspacing (журнальные подписи)
- Появление карточки — `SectionReveal` (переиспользуем)
- CTA-кнопка: чёрный pill (`bg-fg text-bg-elevated`, hover `bg-fg-soft`), при loading — спиннер + disabled
...
- `/chat`: тот же бумажный стиль, крупное сериф-приветствие, mono-бейдж «в разработке»
- Header лендинга auth-aware: при наличии `ibo_user` cookie (клиентская проверка) кнопки «Войти» / «Попробовать ibo» скрываются, вместо них pill «Открыть чат» → `/chat`
