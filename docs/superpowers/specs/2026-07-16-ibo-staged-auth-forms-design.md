...
Все поля визарда живут в одном `<form>`, поэтому нативный сабмит (клик по кнопке `type="submit"` ИЛИ Enter в текстовом поле) всегда всплывает до `<form onSubmit>`. Кнопка «Далее» — тоже `type="submit"`, поэтому Enter и клик по кнопке ведут себя одинаково на любом шаге.

**Важно:** `handleSubmit(callback)` из react-hook-form валидирует ВЕСЬ zod-schema (поля всех шагов), а не только поля текущего шага, и вызывает `callback` только если полная схема прошла валидацию. Если завернуть ветвление по шагам внутрь `handleSubmit(async (values) => { if (step < LAST_STEP) {...} ... })`, на шаге 0 форма никогда не продвинется дальше — поля следующих шагов (например, ещё пустой `password`) провалят полную валидацию схемы, и `callback` с логикой перехода просто не будет вызван. Поэтому обработчик сабмита должен ветвиться ДО обращения к `handleSubmit`, а не внутри него:

```ts
// сетевой сабмит вынесен в отдельную функцию — вызывается только на последнем шаге
const submitStep = async (values: FormValues) => {
...
const onFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  await handleSubmit(submitStep)()
}
```

На промежуточных шагах `trigger(FIELDS_PER_STEP[step])` валидирует только поля текущего шага (RHF выставит ошибки на них при неудаче) — полная схема здесь не участвует. На последнем шаге `handleSubmit(submitStep)()` включает полную валидацию схемы, что корректно, поскольку к этому моменту все поля уже заполнены.
